import {submissionOf, correctionOf, optionalHomeworkOf} from "./homeworkInstructions.js";
import {isNoHomework} from "./noHomework.js";

export const HOMEWORK_CHANGE_DURATION = 120_000;
const fields = {title: "标题", content: "正文", dueAt: "截止时间", subject: "科目", targets: "班级", noHomework: "作业状态", materials: "需带物品", materialsDate: "携带日期", submission: "提交说明", optionalContent: "选做内容"};
...
export function homeworkChangeSnapshot(items, date, workspaceIds) {
  ...
  return items.filter(item => item.type === "ASSIGNMENT" && item.status === "PUBLISHED"
    ...
    .map(item => ({id: item.id, revision: item.revision, title: item.title || "", content: item.content || "",
      ...
      optionalContent: optionalHomeworkOf(item), submission: submissionOf(item), correctionReason: correctionOf(item),
