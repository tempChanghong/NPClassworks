...
test("screen layout preserves dock access, full long homework and expandable context across viewport sizes", async ({browser, request}, testInfo) => {
  ...
  try {
    ...
    for (const position of ["left", "center", "right"]) {
      await page.evaluate(position => localStorage.setItem("classworks-v2-screen-display:screen-a", JSON.stringify({actionPosition: position})), position);
      await page.reload();
    ...
    for (const width of [1920, 1366, 540]) {
      ...
      await expect.poll(() => page.evaluate(() => {
        ...
        return separate && document.documentElement.scrollWidth <= window.innerWidth && bar.left >= 0 && bar.right <= window.innerWidth && bottom < bar.top;
      })).toBe(true);
    }
