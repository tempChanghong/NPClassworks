...
最新实测（2026-09-12）：当前九个浏览器全链路场景及单会话 PostgreSQL 回归均通过，0 失败、0 跳过。新增名单场景与真实旧版升级、性能验收入口见[1.1.0 发布后验收](./release-acceptance-v1.1.0.md)。文末保留此前各轮的实测记录。
...
下列三个是最初的主流程。当前套件另覆盖跨标签通知回执、通知历史恢复目标、超过一百条的 feed、通知认证和撤权、五种通知等级/弹窗组合，以及管理员与大屏名单冲突，共九个浏览器用例。
