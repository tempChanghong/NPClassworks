...
- **共享服务器部署**：服务器已有统一 Caddy/Nginx 和其他站点；NPClassworks 不占用 80/443，只向本机回环地址提供两个高位端口。
...
| 项目 | 独占单服务器 | 共享服务器 | 多服务器 |
| --- | --- | --- | --- |
| 部署难度 | 低 | 低至中 | 中高 |
| 公网入口 | 项目内置 Caddy | 服务器现有网关 | 独立网关 |
| 80/443 | 由本项目占用 | 不占用 | 由网关占用 |
| 网络要求 | Docker 内部网络 | 本机回环网络 | 服务器间需可靠私网 |
| 当前脚本支持 | 完整 | 完整 | 需按本文拆分执行 |
| 推荐场景 | 独占主机 | 已托管其他网站 | 已有分布式基础设施 |
...
如果服务器的 80/443 已由公共 Caddy、Nginx 或面板网关占用，不要启动项目内置 Caddy。编辑 `deploy/.env.production`：
...
- `127.0.0.1:13000` → 后端 API 与 Socket.IO；
